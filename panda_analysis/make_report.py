...
def build_ch1_collection(doc):
    ...
    steps = [
        ...
        "userNick、userLevel、ipLocatedName、touristTypeDisplay、"
    ...
    add_heading(doc, "1.3 数据集情况", level=1)
    add_p(doc,
...
def build_ch4_results(doc):
    ...
    add_p(doc,
        ...
        "判别。由于样本按 group×score 分层抽取,未加权比例主要服务于"
        "国内/国际与评分层内部对比,不能直接解释为携程总体自然口碑。"
        "因此本节同时报告未加权分层样本结果和按清洗后总体评分分布"
        "校正后的加权结果。")
